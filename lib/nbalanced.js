...
reversal = require("./nbalanced/reversal"),
event = require("./nbalanced/event"),
marketplace = require('./nbalanced/marketplace');
...
var nbalanced = module.exports = function (options) {
    ...
    this.Reversals = new reversal(options);
    this.Events = new event(options);
    this.Marketplaces = new marketplace(options);
...
account.prototype.balanced = account.prototype.nbalanced = function (account_data) {
	...
	reversals_uri: account_data.reversals_uri || (self._action_uri + "/" + id + "/reversals")
    };
...
customer.prototype.balanced = customer.prototype.nbalanced = function (customer_data) {
	...
	reversals_uri: customer_data.reversals_uri || (accounts_uri_prefix + "/" + id + "/reversals"),
...
module.exports.MakeTestMarket = function(callback) {
    ...
    if(process.env.BALANCED_DEBUG) {
	if(process.env.BALANCED_DEBUG_HOST)
	    request_params.hostname = process.env.BALANCED_DEBUG_HOST;
	if(process.env.BALANCED_DEBUG_PORT)
	    request_params.port = process.env.BALANCED_DEBUG_PORT * 1;
	if(process.env.BALANCED_DEBUG_NO_HTTPS)
	    https = require('http');
    }
