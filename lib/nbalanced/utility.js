...
if(process.env.BALANCED_DEBUG) {
    console.error("*******************************************************");
    console.error("**           Balanced Client in Debug mode           **");
    console.error("*******************************************************");
    if(process.env.BALANCED_DEBUG_NO_HTTPS) {
	console.error("*** BALANCED CLIENT USING HTTP INSTEAD OF HTTPS");
	https = require('http');
    }
}
...
exports.apiCall = function (options, callback) {
	...
	if(process.env.BALANCED_DEBUG_HOST)
	    reqOptions.hostname = process.env.BALANCED_DEBUG_HOST;
	if(process.env.BALANCED_DEBUG_PORT)
	    reqOptions.port = process.env.BALANCED_DEBUG_PORT * 1;
